...
void DAC_Ch1_SimpleConfig(void);
...
static const uint32_t DAC_DHR12R1_ADDRESS = (uint32_t)&(DAC->DHR12R1);
/**
  * @brief   Main program
  * @param  None
  * @retval None
...
void DAC_Ch1_SineWaveConfig(void)
{
  ...
  remcu_store2mem(_RAM_PTR, Sine12bit, sizeof(Sine12bit));
  printf(" Sine12bit 5 -> %d\n", Sine12bit[0]);

  uint16_t buf[39] = {0};

  remcu_loadFrMem(_RAM_PTR, 11, buf);
  ...
  DMA_InitStructure.DMA_Memory0BaseAddr = (uint32_t)_RAM_PTR;
  DMA_InitStructure.DMA_DIR = DMA_DIR_MemoryToPeripheral;
  DMA_InitStructure.DMA_BufferSize = 32;
  DMA_InitStructure.DMA_PeripheralInc = DMA_PeripheralInc_Disable;
  DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;
...
void DAC_Ch1_EscalatorConfig(void)
{
  ...
  printf(" Escalator8bit 5 -> %d\n", Escalator8bit[5]);

  uint8_t buf[39] = {0};

  remcu_loadFrMem(_RAM_PTR, 6, buf);
...
//  DAC_InitStructure.DAC_Trigger = DAC_Trigger_None;
  DAC_InitStructure.DAC_WaveGeneration = DAC_WaveGeneration_Noise;
  DAC_InitStructure.DAC_LFSRUnmask_TriangleAmplitude = DAC_TriangleAmplitude_31; //DAC_TriangleAmplitude_1023;
  DAC_InitStructure.DAC_OutputBuffer = DAC_OutputBuffer_Enable;
  DAC_Init(DAC_Channel_1, &DAC_InitStructure);
