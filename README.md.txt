...
4. Run a Docker Container:
   ...
   ```bash
   docker run -it --name <container_name> -v /path/to/remcu-mcu-sdks:/project <image_name>
   ```
...
3. Get the prebuilt [LLVM ADIN fork]():
   ...
   **or**
   ...
   Build [LLVM ADIN fork]() (optional):
	If you prefer to build LLVM ADIN fork yourself instead of using a prebuilt version, follow the instructions provided in the [ADIN LLVM repository]() to build LLVM ADIN fork.
...
4. **Make** for Windows https://gnuwin32.sourceforge.net/packages/make.htm

- Ninja (optional): Install Ninja, which is an optional build system that can provide faster build times.

5. Prebuilt [LLVM ADIN fork]() or build manually
